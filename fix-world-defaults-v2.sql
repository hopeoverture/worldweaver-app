-- Fix the ambiguous column reference in world defaults function
...
BEGIN
  ...
  -- Get the folder IDs
  SELECT id INTO folder_characters FROM public.folders WHERE world_id = p_world_id AND name = 'Characters' LIMIT 1;
  SELECT id INTO folder_locations FROM public.folders WHERE world_id = p_world_id AND name = 'Locations' LIMIT 1;
  SELECT id INTO folder_items FROM public.folders WHERE world_id = p_world_id AND name = 'Items & Artifacts' LIMIT 1;
  SELECT id INTO folder_organizations FROM public.folders WHERE world_id = p_world_id AND name = 'Organizations' LIMIT 1;
  SELECT id INTO folder_events FROM public.folders WHERE world_id = p_world_id AND name = 'Events & History' LIMIT 1;
  SELECT id INTO folder_lore FROM public.folders WHERE world_id = p_world_id AND name = 'Lore & Mythology' LIMIT 1;
  SELECT id INTO folder_npcs FROM public.folders WHERE world_id = p_world_id AND name = 'NPCs' LIMIT 1;
  SELECT id INTO folder_quests FROM public.folders WHERE world_id = p_world_id AND name = 'Quests & Plot Hooks' LIMIT 1;

  -- Create default card types using the existing template function
  SELECT copy_card_type_template('Character', p_world_id) INTO type_character;
  SELECT copy_card_type_template('Location', p_world_id) INTO type_location;
  SELECT copy_card_type_template('Magic Item', p_world_id, 'Item/Artifact') INTO type_item;
  SELECT copy_card_type_template('Organization', p_world_id) INTO type_organization;
  ...
  SELECT id INTO type_event FROM public.card_types WHERE world_id = p_world_id AND name = 'Event' LIMIT 1;
  SELECT id INTO type_npc FROM public.card_types WHERE world_id = p_world_id AND name = 'NPC' LIMIT 1;
  SELECT id INTO type_quest FROM public.card_types WHERE world_id = p_world_id AND name = 'Quest' LIMIT 1;

  -- Create starter cards to help users get started
  INSERT INTO public.cards (id, world_id, folder_id, type_id, title, name, slug, summary, position) VALUES
    -- Welcome/Getting Started cards
    (gen_random_uuid(), p_world_id, folder_lore, type_event, 'Getting Started', 'Getting Started', 'getting-started', 'Welcome to your new world! This is an example card to help you get started.', 0),
    
    -- Example Character
    (gen_random_uuid(), p_world_id, folder_characters, type_character, 'The Protagonist', 'The Protagonist', 'the-protagonist', 'Your main character - customize this card or create new ones!', 0),
